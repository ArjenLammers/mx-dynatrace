...
1. **Only RN < 0.60**: Execute [`npx instrumentDynatrace`](#npx-instrumentdynatrace) or `react-native instrument-dynatrace` in the root of your React Native project. This will configure both Android and iOS projects with the settings from `dynatrace.config.js`. You can use the same [custom arguments](#custom-arguments-for-instrumentation-script) as mentioned above.

2. Use `react-native run-android` or `react-native run-ios` to rebuild and run your app. Specify custom paths via [custom arguments.](#custom-arguments-for-instrumentation-script).
...
1. Create several dynatrace.config.js (e.g. dynatrace.config.prod.js) and pass those configuration files via [arguments](#custom-arguments-for-instrumentation-script) in the CLI.
