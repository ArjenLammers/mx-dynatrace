...
function _getAssetData() {
  _getAssetData = _asyncToGenerator(function*(
    ...
    // Mendix: Fix production bundling of assets in node_modules when on another drive
...
function _getAsset() {
  _getAsset = _asyncToGenerator(function*(
    ...
    // Mendix: Fix development serving of assets in node_modules when on another drive
    const nodeModulesRoot = watchFolders.find(p => p.endsWith("node_modules"));
    const absolutePath = relativePath.startsWith("node_modules")
      ? path.resolve(nodeModulesRoot, relativePath.substring("node_modules/".length))
